...
const Toast = ({ toast, onClose }) => {
    ...
    return (
        <div
            className={`flex items-start p-4 rounded-lg border shadow-lg min-w-80 max-w-md slide-up ${getStyles()}`}
            style={{ animation: 'slideUp 0.3s ease-out' }}
        >
            <div className="flex-shrink-0">{getIcon()}</div>
            <div className="ml-3 flex-1">
                <p className="font-ui text-sm font-medium">{toast.message}</p>
            </div>
            <button
                onClick={onClose}
